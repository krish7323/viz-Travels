...
router.get("/all", async (req, res) => {
  try {
    // 1. Fetch from database WITHOUT sorting to bypass the 32MB MongoDB limit
    const packages = await Package.find()
      .select({ title: 1, price: 1, location: 1, category: 1, timestamp: 1, images: { $slice: 1 } })
      .lean();

    // 2. Sort safely in JavaScript memory
    packages.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
...
router.get("/my-packages", async (req, res) => {
  try {
    ...
    // 1. Fetch from database WITHOUT sorting
...
// 5. Get a single package by ID (Loads full details for the specific tour page)
